    ...
    public class NyARParam
    {
        ...
        public void loadARParamFromFile(string i_filename)
        {
            try
            {
                ...
                Task<StorageFolder> t = local.GetFolderAsync("Data Folder").AsTask();
                t.Wait();
                var dataFolder = t.Result;
                // Get the file.
                Task<Stream> file = dataFolder.OpenStreamForReadAsync(i_filename);
                file.Wait();

                loadARParam(file.Result);
            }
            catch (Exception e)
            {
