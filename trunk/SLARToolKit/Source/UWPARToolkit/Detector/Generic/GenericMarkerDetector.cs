...
namespace SLARToolKit
{
    ...
    public class GenericMarkerDetector : AbstractMarkerDetector
   {
      ...
      public GenericMarkerDetector(int width, int height, double nearPlane, double farPlane, IList<Marker> markers, bool adaptive = false)
         : this()
      {
         Initialize(width, height, nearPlane, farPlane, markers, adaptive).Wait();
      }
      ...
      /// <param name="markers">A list of markers that should be detected.</param>
      /// <param name="adaptive">Performs an adaptive bitmap thresholding if set to true. Default = false.</param>
      public async Task Initialize(int width, int height, double nearPlane, double farPlane, IList<Marker> markers, bool adaptive = false)
      {
         this.buffer = new XrgbIRaster(width, height);
         await Initialize(width, height, nearPlane, farPlane, markers, XrgbIRaster.BufferType, adaptive);
      }
      ...
      /// <param name="adaptive">Performs an adaptive bitmap thresholding if set to true. Default = false.</param>
      public async Task Initialize(int width, int height, double nearPlane, double farPlane, Marker[] markers, bool adaptive = false)
      {
         await Initialize(width, height, nearPlane, farPlane, new List<Marker>(markers), adaptive);
      }
